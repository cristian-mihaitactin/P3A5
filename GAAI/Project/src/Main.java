...
public class Main {
	...
	private static boolean isGoodSelection(int i, ArrayList<Integer> selection) {
		if (i > 2 && (selection.get(i - 3) == selection.get(i) && selection.get(i - 2) == selection.get(i)
				&& selection.get(i - 1) == selection.get(i)))
			return true;
		if (i < selection.size() - 3 && (selection.get(i + 3) == selection.get(i)
				&& selection.get(i + 2) == selection.get(i) && selection.get(i + 1) == selection.get(i)))
			return true;

		return false;
	}

	private static Pixel[][] getPixelsFromImage(Mat image) {
	...
	private static ArrayList<Double> getColumnsBlackness(Pixel[][] pixels) {
	...
	private static ArrayList<Integer> getColumnsDelimitation(ArrayList<Double> columnsBlackness) {
